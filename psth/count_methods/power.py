...
def plot_compare_dff(res, start_days, end_days, arg, valence, more_stats, figure_path):
    ...
    plt.title(valence)
    if valence == 'CS+':
        plt.text(0, 1, '{:.1f}% diminished'.format(fraction * 100))
    if valence == 'CS-':
        plt.text(0, 1, '{:.1f}% increased'.format(100 - fraction * 100))
    ...
    if more_stats:
        ...
        plt.text(0, .8, 'Of those, {:.1f}% are gone'.format(100 * lost_fraction))
